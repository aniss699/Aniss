...
export const aiService = {
 async analyzeWithAI(data: BriefAnalysisRequest): Promise<AIAnalysisResult> {
  try {
 ...
 async optimizeBrief(data: { description: string }) {
  try {
   ...
   console.error('Erreur lors de l\'optimisation du brief:', error);
   throw error;
  }
 },
 ...
 async detectAbuse(data: { bidData: any }) {
  try {
   const response = await fetch('/api/ai/detect-abuse', {
    method: 'POST',
    headers: {
     'Content-Type': 'application/json',
    },
